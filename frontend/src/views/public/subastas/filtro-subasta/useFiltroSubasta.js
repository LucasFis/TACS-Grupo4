...
const useFiltroSubasta = () => {
  ...
  const aplicar = () => {
    ...
    const key = tipoFigurita === 'SUBASTADA' ? 'idFiguritaSubastada' : 'idFiguritaOfertada'
  ...
  const chipsActivos = () => {
    if (!tipoFigurita || !figuritaSeleccionada) return []
    const label = tipoFigurita === 'SUBASTADA' ? 'Subastada' : 'Ofertada'
    return [{
      campo: 'tipoFigurita',
      valor: tipoFigurita,
...
export default useFiltroSubasta
